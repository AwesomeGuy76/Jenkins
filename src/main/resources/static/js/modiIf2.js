...
$modiIfBt.onclick = async e => {
  ...
  if ($modiIfCurPw.value === '') {
    //현재 비밀번호가 비어있는 경우
    if ($preference.options[index].value) {
      // 장르 선택을 했다면 택한 값을 서버에 넣어준다.
      user.genre = $preference.options[index].value;
    } else if ($modiIfName.value === user.name) {
      // 기존의 이름과 동일한 경우
      $modiIfName.nextElementSibling.textContent = '기존의 이름과 동일합니다.';
      ++errorCount;
    } else if ($modiIfName.value === '') {
      // 이름 인풋창이 비어있는 경우
  ...
  if (checkPwEmty) {
    ...
    if ($modiIfCurPw.value !== modiUser.pw) {
      ...
      $modiIfCurPw.nextElementSibling.textContent =
        '현재 비밀번호와 입력한 비밀번호가 동일 하지않습니다.';
      ++pwErrorCount;
    } else if (
      $modiIfPw.value === modiUser.pw &&
      $modiIfRepw.value === modiUser.pw
    ) {
      // 변경하려는 비밀번호가 기존의 비밀번호와 일치한지 비교
  ...
  localStorage.setItem(
    ...
    JSON.stringify({
      ...
      genre: $preference.options[index].value
        ? $preference.options[index].value
        : user.genre, // 비어있으면 기존의 장르를 넣어준다, 비어있지 않으면 선택한 값을 넣어준다.
